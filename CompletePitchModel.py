...
class PPM:
    ...
    def ppm_model(self, file_name):
        ...
        out_file = os.path.join(out_file_start, "csv_outputs", "Predictions_PPM_" + out_file_name)
...
class PBM:
    ...
    def pbm_model(self, file_name):
        # Load the model
        model = RNNClassifier(input_size=4, hidden_size=32)  # Adjust parameters accordingly
        
        state_start = os.getcwd()
        state_full = os.path.join(state_start, 'model_path', 'PBM.pth')
        model.load_state_dict(torch.load(state_full))
        model.eval()  # Set the model to evaluation mode
        ...
        out_file_name = os.path.basename(file_name)
        out_file_start = os.getcwd()
        out_file = os.path.join(out_file_start, "csv_outputs", "Predictions_PBM_" + out_file_name)
